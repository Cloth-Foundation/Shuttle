...
No Shuttle implementation stage is active. The next scheduled action is the
shared Stage 22.1 manifest and compiler build-protocol contract. Implementation
must not begin until that contract and its explicit start are approved.

## Scheduled work

### Stage 22: Local projects and compiler build protocol

- [ ] **22.1 — Shared contract.** Approve `Shuttle.toml`, package/workspace
  terminology, dependency namespace mapping, schema and protocol versioning,
  compiler request inputs, migration behavior, and diagnostics ownership with
  the Cloth compiler.
