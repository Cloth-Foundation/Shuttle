...
fn main() {
    ...
    if query {
        let capabilities = format!(
            "{{\"schema\":1,\"protocols\":[1,2],\"artifact_formats\":[8],\"compiler_id\":\"{COMPILER_ID}\",\"standard_library\":{{\"package\":\"cloth\",\"version\":\"0.1.0\"}},\"operations\":[\"compile\",\"inspect\",\"link\",\"reuse\"],\"interface_targets\":[\"wasm32\",\"x86_64\"],\"object_targets\":[\"x86_64\"]}}"
        );
        match mode.as_str() {
            "query-version" => println!("{{\"schema\":1,\"protocols\":[1]}}"),
            "query-old-artifact-format" => println!(
                "{}",
                capabilities.replace("\"artifact_formats\":[8]", "\"artifact_formats\":[7]")
            ),
...
fn emit_receipt(arguments: &[std::ffi::OsString]) {
    ...
    let receipt = format!(
        "{{\"schema\":1,\"artifact_format\":8,\"artifact_id\":\"{ARTIFACT_ID}\",\"kind\":\"{kind}\",\"package\":{{\"name\":\"{package}\",\"version\":\"{version}\"}},\"target\":\"{target}\",\"compiler_id\":\"{COMPILER_ID}\",\"dependencies\":[{}]}}",
        dependencies.join(",")
    );
    ...
    match env::var("SHUTTLE_STUB_MODE").as_deref() {
        ...
        Ok("receipt-old-artifact-format") => println!(
            "{}",
            receipt.replace("\"artifact_format\":8", "\"artifact_format\":7")
        ),
